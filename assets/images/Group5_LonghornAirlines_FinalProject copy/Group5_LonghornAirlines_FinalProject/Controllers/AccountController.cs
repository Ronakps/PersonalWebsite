...
namespace Group5_LonghornAirlines_FinalProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //        Zip = rvm.Zip,
        //        State = rvm.State,
        //        ActiveStatus = rvm.ActiveStatus,
        //        //Age = rvm.Age,
        //        AdvantageNumber = rvm.AdvantageNumber,
        //        Miles = rvm.Miles
        ...
        public async Task<ActionResult> Register(RegisterViewModel rvm)
        {
            ...
            {
                // Return the view with the current model to display form errors
                return View(rvm);
            }

            int advantageNumber = await Utilities.GenerateNextAdvantageNumber.GetNextAdvantageNumber(_context, _userManager);
        ...
        public async Task<ActionResult> Login(LoginViewModel lvm, string? returnUrl)
        {
            ...
            {
                // If the user is inactive, refuse login and display an error message
                ModelState.AddModelError("", "Sorry, you have been banned from Longhorn Airlines. Bye!");
                return View(lvm);
            }

            // Attempt to sign the user in using the SignInManager
            Microsoft.AspNetCore.Identity.SignInResult result = await _signInManager.PasswordSignInAsync(user.UserName, lvm.Password, lvm.RememberMe, lockoutOnFailure: false);
